...
ONLY CHANGE f2016070 to Your ID
*/
object Driver{
  ...
  def main(args:Array[String]){
    ...
    //Dot product
    val res1=f2016070.dotProduct(kernel1,kernel2)

    // println(prod)
    //convolution 
     val res2=f2016070.convolute(image,kernel1,imagesize,kernel1size) 

    // // println(result)

    // //activation 
     val res3=f2016070.activationLayer((x:Double)=>if(x>150) x else 0,image) 

    // // println(result)

    // //single pooling

     val res4=f2016070.singlePooling(max,poolmat,size)
    // //pooling

     val res5=f2016070.poolingLayer(max,image,size) // max is a function

    // //Normalize

     val res6=f2016070.normalise(nmat) 

    // //mixed layer

     val res7=f2016070.mixedLayer(image,kernel1,imagesize,kernel1size,(x:Double)=>x,max,size) // max is a function
...
//  COMMENT NEXT LINE WHEN YOU UNCOMMENT THIS
 val res8=time{f2016070.assembly(image,imagesize,w1,w2,b,kernel1,kernel1size,kernel2,kernel2size,kernel3,kernel3size,size)}
    // val res8=f2016070.assembly(image,imagesize,w1,w2,b,kernel1,kernel1size,kernel2,kernel2size,kernel3,kernel3size,size)
